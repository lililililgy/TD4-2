...
| **startFrame** | `int` | アニメーション再生の**開始フレームインデックス**。特定の範囲のみを再生したい場合はその開始フレームを指定します。（デフォルト: `0`） |
| **endFrame** | `int` | アニメーション再生の**終了フレームインデックス**。特定の範囲のみを再生したい場合はその終了フレームを指定します。`-1` の場合は自動的にスプライトシート全体の最後のフレームが使用されます。（デフォルト: `-1`） |
...
## 6. 複数アニメーションが混在するシートでの使用例

1枚のスプライトシート内にプレイヤー用（例: 0〜4番フレーム）と敵用（例: 5〜9番フレーム）の連番画像が混在している場合、エディタのインスペクタからエンティティごとに再生範囲を制限できます。

### 設定例:

* **プレイヤーエンティティ**の `SpriteAnimation` 設定:
  - `startFrame`: `0`
  - `endFrame`: `4`
  - `isLoop`: `true`
  - プレイヤー用の範囲（`0`〜`4`）のみをループ再生します。

* **敵エンティティ**の `SpriteAnimation` 設定:
  - `startFrame`: `5`
  - `endFrame`: `9`
  - `isLoop`: `true`
  - 敵用の範囲（`5`〜`9`）のみをループ再生します。
