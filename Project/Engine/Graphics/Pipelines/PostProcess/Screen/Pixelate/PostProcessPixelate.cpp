...
PostProcessPixelate::PostProcessPixelate() {}
PostProcessPixelate::~PostProcessPixelate() {}

void PostProcessPixelate::Initialize(ShaderCompiler* shaderCompiler, DxManager* dxm) {

	{
		Shader shader;
		shader.Initialize(shaderCompiler);
		shader.CompileShader(L"Packages/Shader/PostProcess/Screen/Pixelate/Pixelate.cs.hlsl", L"cs_6_6", Shader::Type::cs);
		...
		pipeline_->AddCBV(D3D12_SHADER_VISIBILITY_ALL, 0); // b0: PixelateParams
...
void PostProcessPixelate::Execute(const std::string& textureName, DxCommand* dxCommand, Asset::AssetCollection* assetCollection, EntityComponentSystem* entityComponentSystem, ECSGroup* ecsGroup) {
	...
	if (!tag || !tag->GetPostEffectEnable(PostEffectType_Pixelate)) {
